...
func main() {
	...
	//返回以basepath为基准的相对路径 ✅
	path3, _ := filepath.Rel("/a/b", "/a/b/c/d/../e")
	fmt.Println(path3)

	//将路径使用路径列表分隔符分开，见os.PathListSeparator ✅
	//linux下默认为:，windows下为;
	fmt.Println(filepath.SplitList("/windows:/windows/system"))

	//分割路径中的目录与文件 ✅
	dir, file := filepath.Split("/a/b/c/d.jpg")
	...
	//返回分区名 ✅
	fmt.Println(filepath.VolumeName("/a/b/c"))
